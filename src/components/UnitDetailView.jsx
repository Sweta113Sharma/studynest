...
import { ArrowLeft, FileText, Download, ExternalLink, Sparkles, Loader2, Copy, Check, RefreshCw, Bookmark, Plus, Trash2 } from 'lucide-react'
...
export default function UnitDetailView() {
  ...
  const handleAddVideo = (e) => {
    ...
    if (!newVideoTitle.trim() || !newVideoChannel.trim() || !newVideoUrl.trim()) {
      setFormError('Please fill out all fields.')
      return
    }

    const isYt = newVideoUrl.includes('youtube.com') || newVideoUrl.includes('youtu.be')
    if (!isYt) {
      setFormError('Please enter a valid YouTube link.')
      return
    }

    addYoutubeLinkToUnit(selectedSubject.id, selectedUnit.title, {
      title: newVideoTitle.trim(),
      channel: newVideoChannel.trim(),
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'youtube' && (
          ...
          >
            ...
            <div className="glass-card rounded-2xl p-4 border border-slate-300 dark:border-white/10">
              {!showAddVideoForm ? (
                <button
                  onClick={() => setShowAddVideoForm(true)}
                  className="w-full py-3.5 border-2 border-dashed border-slate-300 dark:border-white/15 hover:border-amber-500/50 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 hover:text-amber-600 dark:hover:text-amber-400 transition-all flex items-center justify-center gap-2 cursor-pointer"
                >
                  <Plus className="w-4 h-4" />
                  <span>Suggest or Add a YouTube Video Tutorial</span>
                </button>
              ) : (
                <form onSubmit={handleAddVideo} className="space-y-3 text-left">
                  <h4 className="text-xs font-bold text-slate-900 dark:text-white flex items-center gap-1.5">
                    <ExternalLink className="w-4 h-4 text-red-605" />
                    Add Video Resource
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-750 dark:text-slate-300">Video Title</label>
                      <input
                        type="text"
                        required
                        value={newVideoTitle}
                        onChange={(e) => setNewVideoTitle(e.target.value)}
                        placeholder="e.g. Backpropagation Step-by-Step"
                        className="w-full bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3 py-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-amber-500"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[10px] font-bold text-slate-750 dark:text-slate-300">Channel Name</label>
                      <input
                        type="text"
                        required
                        value={newVideoChannel}
                        onChange={(e) => setNewVideoChannel(e.target.value)}
                        placeholder="e.g. StatQuest"
                        className="w-full bg-slate-100 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3 py-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-amber-500"
                      />
                    </div>
                    <div className="space-y-1 sm:col-span-2">
                      <label className="text-[10px] font-bold text-slate-750 dark:text-slate-300">YouTube URL</label>
                      <input
                        type="url"
                        required
                        value={newVideoUrl}
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    <button
                      type="button"
                      onClick={() => { setShowAddVideoForm(false); setFormError(''); }}
                      className="px-3 py-1.5 text-xs font-bold border border-slate-300 dark:border-white/10 hover:bg-slate-150 rounded-lg"
                    >
                      Cancel
                    </button>
                    <button
                      type="submit"
                      className="px-4 py-1.5 text-xs font-bold bg-amber-600 hover:bg-amber-700 text-white rounded-lg shadow-sm"
                    >
                      Save Link
                    </button>
                  </div>
            ...
            <div className="space-y-3">
              {!selectedUnit.youtube || selectedUnit.youtube.length === 0 ? (
                ...
                selectedUnit.youtube.map((video, i) => (
                  <motion.div
                    key={i}
                    className="glass-card rounded-2xl p-4 border border-slate-300 dark:border-white/10 flex items-center gap-4 hover:border-amber-500/40 transition-all group focus-visible:ring-2 focus-visible:ring-amber-500 relative"
                    initial={{ opacity: 0, x: -15 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: i * 0.08 }}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        <h3 className="font-bold text-slate-900 dark:text-white text-base truncate pr-8">{video.title}</h3>
                    ...
                    <div className="flex items-center gap-2">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-slate-500 dark:text-slate-400 hover:text-red-600 font-bold p-1 text-sm"
                      ...
                      <button
                        type="button"
                        onClick={() => deleteYoutubeLinkFromUnit(selectedSubject.id, selectedUnit.title, video.url)}
                        className="p-1.5 rounded-lg text-slate-400 hover:text-red-650 hover:bg-red-500/10 transition-colors cursor-pointer"
